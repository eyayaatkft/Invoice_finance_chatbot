...
import { getActiveKnowledgeBaseUrl, setActiveKnowledgeBaseUrl } from "../../utils/activeKnowledgeBase";
...
export default function HelpAndSupportPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/knowledge/list`)
      .then(res => res.json())
      .then(data => {
        if (data.urls && data.urls[currentUrl]) {
          setScraped(true);
        } else {
          setScraped(false);
          setLoading(false); // Immediately stop loading if no KB
        }
      })
      .catch(() => {
        setScraped(false);
  ...
  useEffect(() => {
    if (!currentUrl || scraped === false) return;
    fetch(`/api/help-themes?url=${encodeURIComponent(currentUrl)}`)
      .then((res) => res.json())
      .then((data) => {
